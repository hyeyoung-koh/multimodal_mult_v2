...
def load_bert(bert_path, device): #origin
    bert_config_path = os.path.join(bert_path, 'config.json') #config파일: 모델의 모든 파라미터
    bert = BertModel(BertConfig(vocab_size=30797,**load_json(bert_config_path))).to(device)
...
class TransformerBlock(nn.Module, ABC):
    ...
    def forward(self,
        ...
        In the original paper each opera
        tion (multi-head attention or FFN) is
