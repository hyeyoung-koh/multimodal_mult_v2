...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path',type=str,default='F:/0503혜영/data_sep1')
    ##parser.add_argument('--data_path2', type=str, default='D:/seperate/data_sep2')
    #parser.add_argument('--data_path2', type=str, default='F:/0503혜영/data_sep2')
    ...
    parser.add_argument('--batch_size', type=int, default=8) #원래는 64
    ...
    with open(os.path.join(args_.save_path, 'C:/Users/hyeyoung/PycharmProjects/test/mult2/korean_audiotext_transformer/result/config.json'), 'w') as fp:
        json.dump(args_.__dict__, fp, indent=4)

    # seed and device setting
    set_seed(args_.seed)
    os.environ["CUDA_VISIBLE_DEVICES"] = args_.cuda.split(':')[-1]
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') #origin
    args_.device = device
